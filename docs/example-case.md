...
这是一份可以复制后交给 `tools/uploader.html` 校验的案件 JSON 模板。正式入库时系统会自动生成 `case-xxx` ID 和文件名。
...
```text
clues 面向玩家展示。
rules 面向 Validator、Solver 和 Uploader。
...
把案件 JSON 粘贴到 `tools/uploader.html`，确认解析、格式、答案、唯一解验证全部通过后，点击“加入案件库”。
...
确认新案件全部通过并加入案件库后，再打开 `tools/validator.html` 和游戏首页检查。
